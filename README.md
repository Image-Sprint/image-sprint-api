...
- **선택 이유**:
  ...
  - Spring Web + 멀티스레드 방식으로도 구현 가능하지만, 다음과 같은 문제가 있습니다:
    ...
    - 이미지 처리 로직은 대부분 I/O 지연(S3, 파일 디스크 등)에 묶여 있어, 스레드 낭비가 큼
  - Kotlin Coroutine은 비동기 처리에도 코드가 간결하고 가독성이 좋으며, 스레드 효율이 뛰어남
    - 예: 이미지 100장을 동시에 처리해도 thread 1~2개로 처리 가능
...
- 워커 서버:Kotlin + Coroutine + R2DBC 선택 이유:
  - Job 단위로 수십~수백 장의 이미지 변환을 동시에 처리해야 하는 고동시성 환경 
  - Coroutine 기반 구조는 I/O에 최적화되어 thread 1~2개로도 수백 장 병렬 처리 가능 
  - Reactor 대비 절차지향에 가까운 코드 스타일을 유지하며 비동기 로직 구현 가능 → 가독성과 유지보수성 향상
